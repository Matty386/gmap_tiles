...
class GmapRender:
    def __init__(self):
        self._download_queue = (
            {}
        )  # { (url,filename):status, ... } #status = 0, or timestamp when failed
        ...
        try:
            self._download_ua = UserAgent()
        except:
            self._download_ua = "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_9; us-at) AppleWebKit/533.23.1 (KHTML, like Gecko) Version/5.0.5 Safari/533.21.3"
        ...
        self._coords_bound = [
            (0, 0),
            (0, 0),
        ]  # bounding lat-long pair which specifies downloading range
        self._zoom = 0
        # orient image top to this angle in [0,360) degrees
        self._heading_overlay = (
            {}
        )  # {'deg':0, 'position_px':(x,y), 'size':(wid,hei)} #top-left-ref
    ...
    def setVals(
        ...
        coords=None,
        zoom=None,
        heading_overlay=None,
        image_overlays=None,
        image_dir=None,
        threads_max=None,
        retry=None,
        compass_filename=None,
    ):
        """
    ...
    def update(self):
        ...
        if self._new_setting == True:  # need to update?
            ...
            try:
                ...
                self._image_overlays["order"] = list(
                    self._image_overlays["sources"].keys()
                )
            ...
            self._image_temp = self._mergeLayers(
                overlays
            )  # px size: TILE_SIZE * (tiles_wid,tiles_hei)
            self._image_temp = self._orientTemp(
                self._zoom, self._image_temp
            )  # px size: (diameter, diameter)
    ...
    def checkWorkers(self):
        ...
        for ind in reversed(range(len(self._download_threads))):
            if not (self._download_threads[ind].isAlive()):  # assert job is completed
                self._download_threads[
                    ind
                ].join()  # should not block, because job is complete
        ...
        while (len(self._download_threads) < self._download_threads_max) and (ind >= 0):
            ...
            if time() - val > self._download_retry_after:  # only retry after timeout
                try:
                    ua_str = self._download_ua.random
                except:
                    ua_str = self._download_ua
                url, filename = key
                id = len(self._download_threads)
                self._download_threads_out.append(None)
                t = Thread(
                    target=self._worker,
                    args=(url, ua_str, filename, id, self._download_threads_out),
                )
                t.start()
    ...
    def px2latlon(self, coord_xy):
        ...
        if type(y) != type(int(0)):
            return None
        if (x < 0) or (x >= self._res[0]):
            return None
        if (y < 0) or (y >= self._res[1]):
            return None
        ...
        print(
            "MSG -- Center (zoom) (lat,lon) (x,y) (x_abs,y_abs)",
            (self._zoom),
            (c_latlon),
            (c_px),
            (c_abspx),
        ...
        print(
            "MSG -- Target (zoom) (lat,lon) (x,y) (x_abs,y_abs)",
            (self._zoom),
            (t_latlon),
            (x, y),
            (t_abspx),
        ...
        abs_px_offset = (
            center_abs_px[0] - center_norm_px[0],
            center_abs_px[1] - center_norm_px[1],
        )
    ...
    def latlon2px(self, xxx_todo_changeme1):
        ...
        target_norm_px = (
            target_abs_px[0] - abs_px_offset[0],
            target_abs_px[1] - abs_px_offset[1],
        )
        if (target_norm_px[0] < 0) or (target_norm_px[0] >= self._res[0]):
            return None
        if (target_norm_px[1] < 0) or (target_norm_px[1] >= self._res[1]):
            return None
    ...
    def _findTiles(self, default_radius=1.0):
        ...
        if len(self._coords_geo) > 1:
            radius_km = latlonRadius(
                coord_center, self._coords_geo, self._sphere_radius
            )
        else:
            radius_km = float(default_radius)
        # print 'MSG -- _findtiles() radius_km %s' % (radius_km)
        coord_a, coord_b = boundingCoordinates(
            coord_center, radius_km, self._sphere_radius
        )
        self._coords_bound = [coord_a, coord_b]
        radius_px = int(
            math.ceil(
                math.sqrt(self._res[0] * self._res[0] + self._res[1] * self._res[1])
                / 2.0
            )
        )
        # print 'MSG -- _findTiles radius_px ', radius_px
        bound_size = (
            int(math.ceil(math.sqrt(8) * radius_px)),
            int(math.ceil(math.sqrt(8) * radius_px)),
        )  # find a square to bound a circle about the required radius_px box (to allow for image rotation)
    ...
    def _genFilename(self, layer_uid, xxx_todo_changeme3):
        ...
        if len(d) > 0:
            if d[len(d) - 1] != os.sep:
                d += os.sep
        return (
            d + "_".join([str(layer_uid), str(zoom), str(x), str(y)]) + "." + str(ext)
        )
        ...
        for x in range(min(x_range), max(x_range)):
            for y in range(min(y_range), max(y_range)):
                ...
                try:
                    ...
                    try:
                        self._download_queue[(url, filename)]
                    except:
                        self._download_queue[(url, filename)] = 0
    ...
    def _orientTemp(self, zoom, in_image):
        ...
        min_abspx = latlon2abspx(
            zoom, self._coords_bound[0][0], self._coords_bound[0][1]
        )
    ...
    def _orientOutput(self, in_image):
        ...
        """
        try:
            angle = self._heading_overlay["deg"]
        except:
            angle = 0
        center_coord = latlonCenter(self._coords_bound)
        radius_px = int(
        ...
        try:
            ...
            angle = 0
        w, h = (
            self._image_temp_center
        )  # circle of points center, not necessarily image center
...
def main():
    ...
    * (opt) Multiple sources as overlays, order may be specified.
    ...
    renderObj.setVals(
        res=resolution,
        coords=coord_list,
        image_overlays=sources,
        heading_overlay=compass,
        compass_filename="./Compass_rose_transparent.png",
    )
    ...
    print(
        ...
        latlon_point,
        "is the pixel",
        renderObj.latlon2px(latlon_point),
    )
    img.show()
