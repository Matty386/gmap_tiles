...
def abspx2latlon(zoom, x, y):
    ...
    y_val = y
    while x_val < 0:
        x_val += TILE_SIZE * 2 ** (zoom)  # globe-wrap protection
    while y_val < 0:
        y_val += TILE_SIZE * 2 ** (zoom)
    ...
    expo = (y_val - (TILE_SIZE * (2 ** (zoom - 1)))) / float(
        (-TILE_SIZE * (2 ** zoom)) / (2.0 * math.pi)
    )
...
def latlon2abspx(z, lat, lon):
    ...
    y = (
        -(
            0.5
            * math.log(
                (1 + math.sin(math.radians(lat))) / (1 - math.sin(math.radians(lat)))
            )
            / math.pi
            - 1
        )
...
def tileBounds(zoom, coord0, coord1):
    ...
    tile_count = (
        abs(tile_stop[0] - tile_start[0]) + 1,
        abs(tile_stop[1] - tile_start[1]) + 1,
    )  # inclusive difference range
...
def zoomFromCoords(res_box, coord0, coord1, coord_center, max_zoom=20):
    ...
    fits = False
    while (
        (size_px[0] < res_box[0]) or (size_px[1] < res_box[1]) or (fits != True)
    ):  # while output size not big enough, and not fit
        ...
        center_px = latlon2abspx(
            zoom, coord_center[0], coord_center[1]
        )  # absolute px coordinates
        center_px = (
            center_px[0] - res_min[0],
            center_px[1] - res_min[1],
        )  # offset into self._image_temp px coordinates
        offset_px = (
            center_px[0] - (res_box[0] / 2),
            center_px[1] - (res_box[1] / 2),
        )  # differences between self._image_temp and res_box (0,0)
        max_px = (
            center_px[0] + offset_px[0],
            center_px[1] + offset_px[1],
        )  # max needed pixel to test vs size_px
...
def main():
    ...
    for i in range(tests_len):
        ...
        val = latlon2abspx(zoom, coord[0], coord[1])
        print("MSG -- Lat-lon to pixel: val, expected ", val, px)
        assert val == px
        val = latlon2xy(zoom, coord[0], coord[1])
        print("MSG -- Lat-lon to tile: val, expected ", val, tile)
        assert val == tile
        val = abspx2latlon(zoom, px[0], px[1])
        delta = ((val[0] - coord[0]), (val[1] - coord[1]))
        print("MSG -- Pixel to lat-lon: val, expected ", val, coord)
        assert (delta[0] < px_err[0]) and (delta[1] < px_err[1])
        val = xy2latlon(zoom, tile[0], tile[1])
        delta = ((val[0] - coord[0]), (val[1] - coord[1]))
        print("MSG -- Tile to lat-lon: val, expected ", val, coord)
        assert (delta[0] < tile_err[0]) and (delta[1] < tile_err[1])
        val = (px[0] / TILE_SIZE, px[1] / TILE_SIZE)
        print("MSG -- Absolute pixels to tile index: val, expected", val, tile)
        assert val == tile
    print("")
